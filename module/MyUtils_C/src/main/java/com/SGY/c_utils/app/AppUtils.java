...
public final class AppUtils {
    ...
    public static void openApk(Context context, String authority, File file) {
        if (Build.VERSION.SDK_INT == Build.VERSION_CODES.KITKAT || Build.VERSION.SDK_INT == Build.VERSION_CODES.KITKAT_WATCH) {
            SGYSDCardUtils.chmod("777", file.getParentFile());
            SGYSDCardUtils.chmod("777", file);
        }
    ...
    public static String getAppName() {
        try {
            ...
            return SGYUtils.getContext().getResources().getString(labelRes);
        } catch (Exception e) {
            SGYLogUtils.e(e);
        }
        return null;
    ...
    public static ApplicationInfo getMetaData()
            throws Exception {
        PackageManager packageManager = SGYUtils.getContext().getPackageManager();
        return packageManager.getApplicationInfo(SGYUtils.getContext().getPackageName(),
    ...
    public static boolean isActivityTop(String name) {
        ActivityManager manager = (ActivityManager) SGYUtils.getContext().getSystemService(Context.ACTIVITY_SERVICE);
    ...
    public static boolean isTopActivity() {
        boolean isTop = false;
        ActivityManager am = (ActivityManager) SGYUtils.getContext().getSystemService(Context.ACTIVITY_SERVICE);
        ComponentName cn = am.getRunningTasks(1).get(0).topActivity;
        //这里MainActivity会一直处于栈顶 特殊处理一下
        if (cn.getPackageName().equals(SGYUtils.getContext().getPackageName())) {
            isTop = true;
        }
        return isTop;
    }
    ...
    public static void bringToFront() {
        ActivityManager activtyManager = (ActivityManager) SGYUtils.getContext().getSystemService(Context.ACTIVITY_SERVICE);
        List<ActivityManager.RunningTaskInfo> runningTaskInfos = activtyManager.getRunningTasks(Integer.MAX_VALUE);
        for (ActivityManager.RunningTaskInfo runningTaskInfo : runningTaskInfos) {
            if (SGYUtils.getContext().getPackageName().equals(runningTaskInfo.topActivity.getPackageName())) {
