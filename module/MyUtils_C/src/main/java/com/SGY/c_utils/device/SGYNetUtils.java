 ...
 * 网络相关工具类
 */
public class SGYNetUtils {

    /**
     * 判断是否有可用网络
     */
    public static boolean isNetworkAvailable() {
        ConnectivityManager cm = (ConnectivityManager) SGYUtils.getContext().getSystemService(Context.CONNECTIVITY_SERVICE);
    ...
    public static String getIpAddress() {
        if (SGYNetUtils.isNetworkAvailable()) {
            if (SGYNetUtils.isWifi()) {
                WifiManager wifiManager = (WifiManager) SGYUtils.getContext().getApplicationContext().getSystemService(Context.WIFI_SERVICE);
                WifiInfo wifiInfo = wifiManager.getConnectionInfo();
                String ipAddress = SGYStringUtils.intIp2StringIp(wifiInfo.getIpAddress());//得到IPV4地址
                return ipAddress;
            } else {
                try {
    ...
    public static String getWifiSSID() {
        ...
        if (TextUtils.isEmpty(ssId) || ssId.toLowerCase().contains("unknown")) {
            WifiManager wifiManager = (WifiManager) SGYUtils.getContext().getApplicationContext().getSystemService(Context.WIFI_SERVICE);
            WifiInfo wifiInfo = wifiManager.getConnectionInfo();
            if (ActivityCompat.checkSelfPermission(SGYUtils.getContext(), Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
                return "";
            }
